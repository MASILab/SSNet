...
class Trainer(object):
    ...
    def __init__(self, cuda, model, optimizer,loss_fun,
                ...
                compete = False,onlyEval=False, noLSGAN = True):
        self.cuda = cuda
    ...
    def train(self):
        self.model.train()
        out = osp.join(self.out, 'visualization',self.view)
        mkdir(out)
        ...
        for batch_idx, (data, target, target2ch, sub_name, view, img_name) in tqdm.tqdm(
            ...
            if self.GAN:
                ...
                if self.noLSGAN == True:
                    loss = loss_G_L2 + loss_G_GAN / 1000
                else:
                    loss = loss_G_L2 + loss_G_GAN / 100
                ...
                # fv.write('--- epoch=%d, batch_idx=%d, G_loss=%.4f, L2_loss = %.4f \n' % (
                #     self.epoch, batch_idx, loss_G_GAN.data[0],loss_G_L2.data[0] ))
                loss.backward()
                self.optim.step()
                ...
                if batch_idx % 10 == 0:
                    print('--- epoch=%d, batch_idx=%d, D_loss=%.4f, G_loss=%.4f, L2_loss_loss = %.4f  \n' % (
                        self.epoch, batch_idx, self.loss_D.data[0], loss_G_GAN.data[0], loss_G_L2.data[0]))
                    # print('--- epoch=%d, batch_idx=%d, G_loss=%.4f, L2_loss_loss = %.4f  \n' % (
                    # self.epoch, batch_idx, loss_G_GAN.data[0],loss_G_L2.data[0] ))
            else:
                self.optim.zero_grad()
                if self.loss_fun == 'cross_entropy':
